...
g.rect(12, 14, 8, 3)
g.rect(16, 6, 3, 2)
build('導きの回廊', '足音が地図になる', 'ドラッグして歩く。跳ね返った音が壁を刻み、地図になって残る。',
      g, {'P': (2, 2), 'E': (29, 18)})
...
build('水の間', '息をひそめて', '〈息〉を押している間は静かに歩ける。水たまりは大きな音を立てる。',
...
build('巣', '鼓動が漏れる', '橙の波は化け物。怖がるほど鼓動が速くなり、その音まで届いてしまう。',
...
build('迷路', '呼べば、届く', '〈呼〉は遠くまで届く。ただし、あちらにも同じだけ届く。',
...
build('深淵', 'すべてが此処にいる', '奴らは時々立ち止まり、耳を澄ます。音が消えたら、近い。',
      g, {'P': (2, 2), 'E': (37, 26),
...
with open('/home/user/keelpop.github.io/breathless/js/levels.js', 'w') as f:
    f.write('\n'.join(out))
